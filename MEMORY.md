...
1. ~~**답장 유실 창이 24시간이다.**~~ **터졌고, 고쳤다 (2026-08-08).** "데이터 손실은 없고 사람이 다시 답장하면 된다"고 적어 두고 받아들였던 항목이다 — 실제로는 2026-08-06의 `승인 #P0804`가 그대로 유실됐고, 사람은 "인식되지 않았다"만 보았지 왜인지는 알 수 없었다(로그도 침묵했다). 하루 1회 폴링을 **15분 폴링**으로 바꿨다: §9의 「인박스를 15분으로 내린 이유」. 남은 잔여 위험은 텔레그램 API가 23시간 이상 연속으로 죽는 경우뿐이다.
...
| 잡 이름 | 시각 (KST) | `<파일명>` |
|---|---|---|
| econblog-inbox | **15분마다** (`*/15 * * * *`) | `inbox.yml` |
| econblog-daily | 매일 01:30 | `daily-collect.yml` |
| econblog-weekly | 일 01:20 | `weekly-collect.yml` |

`econblog-inbox`는 본문에 입력을 싣지 않는다 — `{"ref":"main"}` 그대로다. `alert_backlog`
기본값이 `false`라 적체 경보는 `econblog-daily` 경로에서만 나간다.

### 인박스를 15분으로 내린 이유 (2026-08-08)
...
폴링을 15분으로 내리면 한 회차를 놓쳐도 남는 여유가 23시간 이상이다. 웹훅이 근본
해법이지만 공개 HTTPS 엔드포인트가 필요하고 이 저장소에는 그것을 둘 자리가 없다 —
cron-job.org가 `workflow_dispatch`를 거는 지금 구조에서 폴링 주기가 유일한 손잡이다.
...
2. ~~**텔레그램 24시간 보존 창에 여유가 생긴다**(§8의 1번).~~ **틀렸다 (2026-08-08).**
   ...
   러너 대기로 취소되자 예산이 통째로 날아갔다. 판정 회수는 이제 `daily-collect`의
   01:30 회차가 아니라 15분마다 도는 `inbox.yml`이 한다 — 위 흐름도의 01:30 인박스는
   **순서를 위한 호출**이지 유일한 회수 경로가 아니다.

~~대가는 하나뿐이다: **01:30~06:00 KST 사이에 보낸 답장은 하루 늦게 처리된다.**~~
해소됐다 — 어느 시간대에 답장하든 15분 안에 처리된다.
