...
**재질의는 크론 잡을 늘리지 않는다.** `daily-collect.yml`의 `reask` 잡이 인박스 뒤에
붙어 있어 `econblog-daily` 한 번에 같이 돈다(2026-08-08 결정). 외부 스케줄러에 잡을
더 만들수록 저장소 밖에 흩어진 설정이 늘고, 그중 하나가 조용히 꺼지면 알 방법이 없다.
