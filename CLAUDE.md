...
A Hugo static-site blog (theme: PaperMod, git submodule at `themes/PaperMod`) explaining Korean economic news to non-expert readers, deployed to GitHub Pages. The product is (a) the Hugo content/config and (b) the `/daily-post` slash command that generates that content.

There is no application code **in the render path** — nothing the site serves is generated by code we wrote. There *is* Python: `.claude/loop/*.py` (5 files, writing-style measurement) and `scripts/fetch_ga4.py` · `scripts/fetch_gsc.py` (analytics readers). All of it is offline tooling run by hand or by an agent, always via `.venv/bin/python` — never bare `python`/`python3`, since the system interpreter lacks the Google API packages. None of it runs during `hugo` build or in CI.
...
No linter, and no test suite for the site itself — the site is prompt files + Markdown + Hugo config. The Python tooling is the exception: `.claude/loop/test_extract_features.py` is a golden test (`.venv/bin/python .claude/loop/test_extract_features.py`), and any new measurement helper is expected to ship one. "Correctness" for the slash commands means the prompt text is internally consistent and matches Hugo's content-file conventions, not passing tests. After editing `.claude/commands/*.md` or `.claude/daily-post/*.md`:
...
- **Agent3 (주간 감사, `/weekly-audit`)** — specced, not built. Spec: `.claude/audit/SEED-weekly-audit.md` (v3.3). Six axes, run weekly by hand: ① link integrity ② performance analysis (GA4/GSC, behind a data-sufficiency gate) ③ indexation health ④ system scan ⑤ direction review (portfolio axes D1–D6 + a pre-registered hypothesis ledger) ⑥ numeric integrity. Implementation plans: `docs/superpowers/plans/2026-07-2*-weekly-audit-*.md` (gitignored).
  - **Design, Lighthouse, and performance measurement are deliberately out of scope** — they need a headless browser. Any doc or review claiming the audit covers them is describing an earlier draft. This line is the authority on scope; SEED AC #35 has the agent report a finding whenever the two drift apart again, and the agent is forbidden from editing this file itself (AC #34·#38).
  - **Its output format is already specced; do not redesign it.** The agent writes `.claude/audit/topic-report.md` (sections 잘 되는 주제 / 안 되는 주제 / 좋은 포스트의 조건 + a `생성일` line), which `rank.md` reads as an optional scoring input. Contract pinned at `.claude/audit/README.md`. `topic-report.md` not existing is normal — `rank.md` skips it silently.
