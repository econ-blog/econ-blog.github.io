...
2. **`/write-post`** (`.claude/commands/write-post.md`) — takes one chosen news item and drafts a post. This command is a thin 4-step sequencer, not where the actual writing logic lives:
   ...
   - §3 tells the agent to `Read` and follow `.claude/write-post/draft.md` — does the actual post writing, dictionary-entry creation, and wikilinking, consuming the analysis note from §2. `draft.md` itself delegates all prose style/tone/prohibition rules (no bold, 두괄식, numeric redlines, personalized-advice ban, the 7-item AI-artifact self-review checklist, etc.) to `.claude/write-post/writing-styles.md` via inline path references rather than inlining them — `draft.md` keeps only the structural instructions (file paths, front matter, which analysis-note fields feed which paragraph).
...
- `content/posts/` — one Markdown file per news explainer. Front matter: `title`, `date`, `tags`, `draft`, `source_url` (always the original article URL, verbatim), and optional `related_articles` (list of `{title, url, source}` — external articles on the same topic, gathered by `/write-post` §1.5 via WebSearch, ordered oldest-first; §1.5 excludes same-day articles and prefers older background pieces, so the block reads as run-up context rather than a second copy of the day's wire; omitted entirely when nothing survives selection, never an empty list). Post footer blocks render in this order via `layouts/partials/extend_post_content.html`: internal related posts → external `related_articles` → source link → disclaimer. Internal first is deliberate — dwell time is the only meaningful pre-AdSense signal. External links carry `rel="nofollow"` since they appear on every post.
...
- **Scheduled/automatic runs**: `/news-pick` and `/write-post` remain manual-only. **This is no longer the end state** — a merged, unattended `/daily-post` command is specced and planned (`docs/superpowers/specs/2026-07-19-daily-cloud-routine-design.md`, `docs/superpowers/plans/2026-07-19-daily-cloud-routine.md`) to run daily as a cloud routine. It will not replace the two manual commands; all three coexist. Key invariants of that design, if you touch it: it pushes only to `auto/post-YYYY-MM-DD` branches (never `main`), always writes `draft: true`, calls no interactive tools, and aborts without producing anything when the top candidate scores below threshold. The publish gate is not removed, only moved to PR review.
- **Cross-verification via `/delegate agy deep-research`**: idea for `/write-post` §1 — send the topic through the `/delegate` skill's `agy` (Antigravity CLI, Gemini 3.1 Pro High) provider's deep-research capability and use that report for fact cross-checking. Motivation: `/news-pick` sources from Hankyung RSS only, a single-source dependency with no cross-verification channel today. Note: agy was separately evaluated and **rejected** as a way to gather external related-article links — deep-research is a report generator, and `WebSearch` returns URLs more cheaply and directly. It remains open only for fact-checking, which is a different job.
