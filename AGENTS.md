...
<system_context>
  ...
  <environment_constraints>
    ...
    - **샌드박스가 못 하는 일은 「붙여넣을 프롬프트」로 넘긴다.** 웹 조회·브라우저·검색
      ...
      새 세션에 붙여넣고 결과만 가져다준다. **판단까지 넘기는 것이 아니다** — 무엇을 할지는
      네가 정하고 손이 필요한 부분만 넘긴다. 새 세션은 이 대화를 볼 수 없으므로 맥락·대상·
  ...
  <operating_model>
    ...
    **스케줄은 저장소 안에 없다.** GitHub 내장 `schedule`은 발화가 신뢰할 수 없어
    (수 시간 지연·누락) 쓰지 않는다. 워크플로는 전부 `workflow_dispatch`만 갖고 있고
    발화는 **cron-job.org가 REST API로** 건다 — 잡 시각은 KST 기준이다.
    워크플로 파일을 고칠 때 `on:`에 `schedule:`을 되살리지 마라. 반대로 새 주기 작업을
    만들면 cron-job.org에 잡을 추가해야 하며, 그것을 빠뜨리면 **오류 없이 그냥 안 돈다.**
