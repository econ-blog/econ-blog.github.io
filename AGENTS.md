...
## `/audit-local` (감사 후속 · 로컬 대화형 전용)

`.claude/commands/audit-local.md`. 클라우드 샌드박스가 도달하지 못하는 감사 후속만 처리한다 —
③ 색인 제출(GSC·네이버) · ⑥ N1 중 원문에서만 확인되는 기준일 · ⑤ D4 소급 1차 출처 링크 ·
① 러너 도달 불가 의심 링크의 실제 도달성.

**클라우드에서 돌리지 않는다.** 명령 §0이 `curl`로 뉴스 사이트 도달을 프로브해 `000`이면
그 자리에서 중단한다. 거기서 돌리면 매 단계가 "확인 불가"로 끝난다.

입력은 가장 최근 `report/audit-*.md`의 `## 로컬 세션 대기열` 섹션이다. 그 섹션은
`/audit-improvement` §9-2가 `로컬 세션`으로 분류한 항목을 싣는다 — **두 명령이 같은 헤딩
문자열을 보므로 한쪽만 바꾸지 않는다.** 새 파일을 만들지 않으므로 감사 산출물은 다섯 개 그대로다.

`content/` 수정은 승인 게이트를 거쳐 `main` 직행이고, `auto/**`에는 아무것도 밀지 않는다.

## 감사 소견의 세 갈래 (`/audit-improvement` §9-2)

모든 소견은 리포트에 실리기 전에 `Claude 판정` · `사람 판정` · `로컬 세션` 중 하나로 표시된다.
판별은 목록 암기가 아니라 두 질문이다: ① 판정 근거가 저장소 안에서 닫히는가 → `Claude 판정`.
② 아니라면 이 세션이 그 사실을 확인할 수 있는가 → 없으면 `로컬 세션`, 확인은 되지만 무엇을
옳다고 볼지가 정책이면 `사람 판정`.

**`사람 판정`을 `Claude 판정`으로 올리지 않는다.** 어려워서가 아니라 틀렸을 때 되돌리는 비용
때문에 사람에게 있다 — 특히 ② `topic-report.md` 조정치와 1차 출처 호스트 목록은 한 번 틀리면
그 뒤 몇 주의 채점이 그 값 위에서 이뤄진다.
