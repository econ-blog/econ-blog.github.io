...
<runtime_invariants>
  ...
  - **발행 게이트는 둘이고 둘 다 통과해야 한다**: 결정론 검사(N1~N5 · T1~T4 · contracts) `통과` **그리고** `post-reviewer` 검토 `발행 가능`. 한쪽이라도 미통과이거나 검사·검토가 불가능하면 발행하지 않는다. 버리지도 않는다 — `draft: true`로 `main`에 남겨 격주 점검의 Q4(방치 초안)와 `/revise-post`가 처리할 수 있게 한다.
